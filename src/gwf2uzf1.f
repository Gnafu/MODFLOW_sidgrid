...
C-------SUBROUTINE GWF2UZF1RP
      SUBROUTINE GWF2UZF1RP(In, Kkper, Igrid, casc_flg)
C     ******************************************************************
C     READ AND CHECK VARIABLES EACH STRESS PERIOD 
C     VERSION 1.7:  SEPTEMBER 15, 2009
C     VERSION SID&GRID: MARCH 2013
CIac:  the subroutine depends also on the cascading flow flag: casc_flg
C     ******************************************************************
      USE GWFUZFMODULE
      USE GLOBAL,       ONLY: NCOL, NROW, NLAY, IOUT, ISSFLG, IBOUND, 
     +                        HNEW, DELR, DELC, BOTM, LBOTM, PERLEN
C -- Iac:      
      use cascading_module
C -- !!
      IMPLICIT NONE
...
C -- Iac:      
      real :: IncomingFlux, OutgoingFlux, AddedFlux
      real, dimension(:,:), allocatable :: newexcess
...
C      ---> Add (IncomingFlux - OutgoingFlux) to rainfall 
              if (casc_flg.GT.0 .AND. Kkper > 1) then
                call CASCADE_IN(IncomingFlux,nrck, ncck, Kkper,1,Igrid)
                call CASCADE_OUT(OutgoingFlux,nrck, ncck, Kkper,1,Igrid) 
            ...
            !        the cascading flow
              ...
              AddedFlux = IncomingFlux 
                 
              if (AddedFlux >= 0) then
                 FINF(ncck,nrck)=FINF(ncck,nrck) + AddedFlux
                     
              end if
            if (casc_flg.GT.0) then
C            ! Save in buffer CFLFLOW the Volumetric flow 
               CFLFLOW(nrck,ncck) = 
     &                (IncomingFlux-OutgoingFlux)*DELR(ncck)*DELC(nrck)
...
C -----------------
              ...
              ELSE IF ( FINF(ncck, nrck).GT.VKS(ncck, nrck) ) THEN
                ...
                newexcess(ncck, nrck)=   (FINF(ncck,nrck)- 
...
C Iac: Update EXCESPP and if no runoff occurs, reset EXCESPP to 0 for this stress period        
        do nrck = 1, NROW
          do ncck = 1, NCOL 
             if (FINF(ncck, nrck).LT.VKS(ncck, nrck) ) THEN
               ! No runoff in this stress period, reset EXCESPP 
               EXCESPP(ncck, nrck) = 0.0
             else
               EXCESPP(ncck, nrck) = newexcess(ncck, nrck)  
             end if
          end do
        end do

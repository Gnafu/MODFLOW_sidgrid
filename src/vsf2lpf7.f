C Version 2013 -- Iacopo
C This is the new version of file vsf1lpf1.f
C and it is based on gwf2lpf7.7
C The modification with respect to gwf2lpf7.7 are marked by the comment C-VSF
      SUBROUTINE VSF2LPF7AR(IN,IGRID)
C     ******************************************************************
...
C9------RETURN
      ...
      SUBROUTINE VSF2LPF7FM(KITER,KSTP,KPER,IGRID)
C     ******************************************************************
...
C2------FOR EACH LAYER: IF CONVERTIBLE, CALCULATE CONDUCTANCES.
      ...
      IF(LAYTYP(K).NE.0)
C -- VSF      
     1    CALL SVSF2LPF7HCOND(KK,KITER,KSTP,KPER)
  100 CONTINUE
      DO 101 K=1,NLAY
      KK=K
      IF(K.NE.NLAY) THEN
...
C6------WHEN TO USE PRIMARY AND SECONDARY STORAGE
            DO 180 I=1,NROW
            DO 180 J=1,NCOL
C5VSF--- IF CELL TYPE IS VSF/OLF (STYPE NOT EQUAL TO ZERO), CYCLE
            IF (STYPE(J,I,K).NE.0) CYCLE
...
C8A-----FOR EACH CELL MAKE THE CORRECTION IF NEEDED.
           DO 220 I=1,NROW
           DO 220 J=1,NCOL
C8VSF--- IF CELL TYPE IS VSF/OLF (STYPE NOT EQUAL TO ZERO), CYCLE
           IF (STYPE(J,I,K).NE.0) CYCLE           
C
...
C9A-----FOR EACH CELL MAKE THE CORRECTION IF NEEDED.
           DO 280 I=1,NROW
           DO 280 J=1,NCOL
C9VSF--- IF CELL TYPE IS VSF/OLF (STYPE NOT EQUAL TO ZERO), CYCLE
          IF (STYPE(J,I,K).NE.0) CYCLE           
C
...
C5------CALCULATE HOR. CONDUCTANCE(CR AND CC) FOR CONSTANT T LAYERS.
      ...
      KK=K
C --VSF      
      IF(LAYTYP(K).EQ.0) CALL SVSF2LPF7HCOND(KK,0,0,0)
   70 CONTINUE
C
...
C7------RETURN.
      RETURN
      END
      SUBROUTINE VSF2LPF7BDADJ(KSTP,KPER,IDIR,IBDRET,
...
C5B-----FOR EACH CELL CALCULATE FLOW THRU LOWER FACE & STORE IN BUFFER.
      ...
      END IF
      HD=HNEW(J,I,K+1)
CVSF-- VSF/OLF CELLS ARE TREATED AS UNCONFINED
      IF(NOVFC.NE.0.OR.LAYTYP(K+1).EQ.0.OR.STYPE(J,I,K).NE.0) GO TO 580
C     IF(NOVFC.NE.0 .OR. LAYTYP(K+1).EQ.0) GO TO 580
      TMP=HD
      TOP=BOTM(J,I,LBOTM(K+1)-1)
...
C5C-----RECORD CONTENTS OF BUFFER AND RETURN.
     ...
     1     NLAY,IOUT,DELT,PERTIM,TOTIM,IBOUND)
      RETURN
      END
      SUBROUTINE VSF2LPF7BDS(KSTP,KPER,IGRID)
C     ******************************************************************
...
C7A-VSF- IF CELL TYPE IS VSF/OLF (STYPE NOT EQUAL TO ZERO), CYCLE
      IF (STYPE(J,I,K).NE.0) CYCLE      
C
...
C11----RETURN.
      RETURN
      END
      SUBROUTINE VSF2LPF7BDCH(KSTP,KPER,IGRID)
C     ******************************************************************
...
C11-----CALCULATE FLOW THROUGH THE UPPER FACE.
  120 IF(K.EQ.1) GO TO 150
      ...
      HD=HNEW(J,I,K)
CVSF-- IF CELL TYPE IS VSF/OLF (STYPE NOT EQUAL TO ZERO), CYCLE
      IF(NOVFC.NE.0 .OR.LAYTYP(K).EQ.0.OR.STYPE(J,I,K).NE.0) GO TO 122      
C     IF(NOVFC.NE.0 .OR. LAYTYP(K).EQ.0) GO TO 122
      TMP=HD
      TOP=BOTM(J,I,LBOTM(K)-1)
      IF(TMP.LT.TOP) HD=TOP
...
C12-----CALCULATE FLOW THROUGH THE LOWER FACE.
  150 IF(K.EQ.NLAY) GO TO 180
      ...
      HD=HNEW(J,I,K+1)
CVSF-- IF CELL TYPE IS VSF/OLF (STYPE NOT EQUAL TO ZERO), DO NOT ALTER HEAD  
      IF(NOVFC.NE.0 .OR.LAYTYP(K).EQ.0 .OR.STYPE(J,I,K).NE.0)GO TO 152
C      IF(NOVFC.NE.0 .OR. LAYTYP(K+1).EQ.0) GO TO 152
      TMP=HD
      TOP=BOTM(J,I,LBOTM(K+1)-1)
      IF(TMP.LT.TOP) HD=TOP
...
C18-----RETURN.
      RETURN
      END
      SUBROUTINE SVSF2LPF7HCOND(K,KITER,KSTP,KPER)
C     ******************************************************************
...
C3B-----CALCULATE SATURATED THICKNESS FOR A WET CELL.
   ...
   36          FORMAT(1X,'Top elevation, bottom elevation:',1P,2G13.5)
         ...
         THCK=TTOP-BBOT
C
C3VSF-- IF CELL TYPE IS VSF/OLF SET THCK = CELL THICKNESS (THCK=TTOP-BBOT)
	     IF(STYPE(J,I,K).NE.0) THEN
          TTOP=BOTM(J,I,LBOTM(K)-1)
		  THCK=TTOP-BBOT
	     ENDIF
C         
...
C VSF-- DO NOT ADJUST SATURATED THICKNESS FOR VSF/OLF CELLS
               IF(LAYTYP(K)/=0.AND.ICONCV==0.AND.STYPE(J,I,K)==0) THEN
                  HHD=HNEW(J,I,K)
                  IF(HHD.LT.TTOP) TTOP=HHD
               END IF
C               IF(LAYTYP(K).NE.0 .AND. ICONCV.EQ.0) THEN
C                  HHD=HNEW(J,I,K)
...
C5A-----IN THE VERTICAL CONDUCTANCE CALCULATION, EXCEPT THAT THE NOCVCO
C5A-----AND ICONCV FLAGS TURN OFF THIS CORRECTION.               
C
CVSF -- DO NOT ADJUST SATURATED THICKNESS FOR VSF/OLF CELLS
               IF(LAYTYP(K+1).NE.0 .AND.NOCVCO.EQ.0
     &            .AND. ICONCV.EQ.0.AND.STYPE(J,I,K+1).EQ.0) THEN
     
                  HHD=HNEW(J,I,K+1)
                  IF(HHD.LT.TTOP) B=ZERO
               END IF
C              IF(LAYTYP(K+1).NE.0
C     1                    .AND.NOCVCO.EQ.0 .AND. ICONCV.EQ.0) THEN
C                  HHD=HNEW(J,I,K+1)
